...
electronicsRouter.post("/create",uploadFiles, async (req, res) => {
    ...
    try {
        const { title, price,rating,quantity, description:{ brand, operating_system, ram, memory, cpu_model, details }, type } = req.body
        ...
        const createProduct = electronics({
            title, price,image:image?.path,rating, quantity, description:{ brand, operating_system, ram, memory, cpu_model, details }, type
        })
